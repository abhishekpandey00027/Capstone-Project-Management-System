...
UPDATE evaluation
SET ISA1=20, ISA2=20, ISA3=20, ESA=80
WHERE SRN='pes2ug20cs000' AND Semester=6;

-- --------------------------
-- Now add all foreign key constraints after data is inserted
-- --------------------------

-- Add foreign key constraints
ALTER TABLE evaluation
ADD CONSTRAINT `srn_fk` FOREIGN KEY (`SRN`) REFERENCES `student` (`SRN`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE faculty
ADD CONSTRAINT `domain_fk` FOREIGN KEY (`domain_id`) REFERENCES `domain` (`DomainID`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE panel
ADD CONSTRAINT `panel_domain_fk` FOREIGN KEY (`domain_id`) REFERENCES `domain` (`DomainID`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE team
ADD CONSTRAINT `team_domain_fk` FOREIGN KEY (`domain_id`) REFERENCES `domain` (`DomainID`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE team
ADD CONSTRAINT `team_panel_fk` FOREIGN KEY (`panel_id`) REFERENCES `panel` (`panel_id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
-- Note: This requires SUPER or CREATE USER privilege
-- If you get an error, you may need to run this separately with appropriate privileges
CREATE USER IF NOT EXISTS 'capstone_student'@'%' IDENTIFIED BY 'studentstudent';
...
GRANT SELECT, INSERT ON capstones.evaluation TO 'capstone_student'@'%';
...
-- Note: This requires SUPER or CREATE USER privilege
-- If you get an error, you may need to run this separately with appropriate privileges
CREATE USER IF NOT EXISTS 'capstone_faculty'@'%' IDENTIFIED BY 'facultyfaculty';
-- Grant permissions on the necessary tables and columns
...
GRANT SELECT (panel_id, head_id, domain_id) ON capstones.panel TO 'capstone_faculty'@'%';
